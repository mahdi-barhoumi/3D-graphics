...
namespace Engine
{
    struct Color
    {
        ...
        constexpr Color(float intensity)
            : r(static_cast<unsigned char>(std::clamp(intensity, 0.0f, 1.0f) * 255.0f))
            , g(static_cast<unsigned char>(std::clamp(intensity, 0.0f, 1.0f) * 255.0f))
            , b(static_cast<unsigned char>(std::clamp(intensity, 0.0f, 1.0f) * 255.0f)) {}
        ...
        constexpr Color(glm::vec4 color)
            : r(static_cast<unsigned char>(std::clamp(color.r, 0.0f, 1.0f) * 255.0f))
            , g(static_cast<unsigned char>(std::clamp(color.g, 0.0f, 1.0f) * 255.0f))
            , b(static_cast<unsigned char>(std::clamp(color.b, 0.0f, 1.0f) * 255.0f))
            , a(static_cast<unsigned char>(std::clamp(color.a, 0.0f, 1.0f) * 255.0f)) {}
